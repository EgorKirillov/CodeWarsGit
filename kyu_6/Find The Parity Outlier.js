...
function findOutlier(integers) {
    ...
    //odd - нечет , even - чет
    let modulo = ( (Math.abs(integers[0] % 2) +Math.abs(integers[1] % 2) +Math.abs(integers[2] % 2)) > 1)?
      1 :0
    return integers.find(el => Math.abs(el % 2) !== modulo)
}

console.log(findOutlier([ 1, 1, -1, 1, 1, -44, 7, 7, 7, 7, 7, 7, 7, 7 ]))